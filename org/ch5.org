...
We'll start by rewriting =parseToken=. Rather than returning a plain =Token= and failing at runtime if provided an invalid numeric value, we'll use =readMaybe= to yield a =Maybe Int=, returning an =Either= that contains an early-termination function over =Result= values or an integer wrapped by a =Lit= constructor
