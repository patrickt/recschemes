...
This function just got far, far easier to write and maintain. The =apply= function is now responsible for both the base case and the simple recursive case of flattening an expression: all we have to do is define the interesting case, i.e. its handling of =Paren= nodes. Awesome---but's not get ahead of ourselves. We haven't really prevented any boilerplate or eliminated room for bugs here: =applyExpr= just contains and isolates the boilerplate, and we'd need to write a new =apply= function for every new type we define. A sufficiently smart compiler could write them for us. And GHC, being a very smart compiler, can. First, though, we'll have to make this =Expr= data type a little bit more general.
...
In Haskell, we call this combinator ~fix~. We can use ~fix~ to write a recursive function without referring to the function in question recursively: the parameter passed to this function represents itself, and we can use it to recur:

#+BEGIN_SRC haskell
countDown :: Natural -> IO ()
countDown = fix $ \recur n -> do
  putStrLn (show n)
  when (n /= 0) (recur (n - 1)) -- recurring, without mentioning 'countDown'
#+END_SRC
...
We need a data type =Y= that, when given another type =f=, wraps an =f= whose children are of type =(Y f)=. Let's call it =Term=[fn:1], and let's call its constructor =In=, representing the fact that we are stuffing one level of recursion into a fixed form. In addition, we'll define an =out= function that unwraps a =Term=.
...
[fn:1] Some recursion-schemes libraries call this type ~Fix~, reflecting its fixed-point nature (and providing an elegant symmetry with the value-level ~fix~ function).
...
We have the tools to express each step of this procedure---let's call it =bottomUp=. Given a function =fn= mapping from =Term= to =Term=, we'll unpack the =Term= with the =out= function, recursively traverse each child of the unpacked term with =fmap (bottomUp fn)=, repack the term with the =In= constructor, and then simply apply =fn= to the result. The =fmap bottomUp= call does all the heavy lifting in this function: it captures the act of recursing into each child (if any) of a given functor.
