...
Reading \texttt{ ORRRR } as `or' can be a helpful mnemonic: we can see that \texttt{f ORRRR g} returns a function
...
\begin{code}
elgot :: Functor f => Algebra f b -> (a -> Either b (f a)) -> a -> b
elgot alg coalg = coalg >>> (id ORRRR (fmap (elgot alg coalg) >>> alg))
\end{code}
...
In the defintion of \texttt{elgot} above,  we used \texttt{ORRRR} to handle the Either case: in performing
...
being its own dual. The \texttt{ORRRR} may be slightly less obvious, but if we remember that the
...
Whereas \texttt{ORRRR} took two functions and used one or either of them to deconstruct an \texttt{Either},
